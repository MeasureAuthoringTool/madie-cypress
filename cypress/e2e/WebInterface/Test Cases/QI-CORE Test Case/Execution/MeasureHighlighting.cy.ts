...
let measureCQL_withDuplicateLibraryDefinition_wNoUnusedDefine =
    "library Library7027567898767 version '0.0.000'\n\n" +
...
let measureCQL_withDuplicateLibraryDefinition =
    "library Library7027567898767 version '0.0.000'\n" +
    '\n' +
    "using QICore version '4.1.1'\n" +
    '\n' +
...
describe('Measure Highlighting', () => {
    ...
    it('Execute single Test Case and verify Measure highlighting', () => {
        ...
        TestData.readMeasureId().then((fileContents) => {
        ...
        cy.wait('@group', { timeout: 60000 }).then((request) => {
            TestData.writeFixture('groupId', request?.response?.body.id)
        })
        ...
        */
        ...
        cy.wait('@expansion', { timeout: 35000 }).then((expansion) => {
            for (let valueset of expansion?.response?.body) {
                ...
                let duplicates = codes.filter(
                    (value, index) => codes.indexOf(value) !== index && codes.lastIndexOf(value) === index
                )
                // 3
        ...
        TestCasesPage.executeBooleanPassingTestCaseAndOpenHighlighting({
            waitForRunCompletionMs: 25500
        })

        TestData.readMeasureGroupId().then((fileContents) => {
...
describe('Highlighting accurately appears for a single PC measure', () => {
    ...
    it('Verify highlighting for a measure with a single PC', () => {
        ...
        Utilities.waitForElementVisible(MeasureGroupPage.measureGroupTypeSelect, 60000)
        ...
        TestData.readMeasureGroupId().then((fileContents) => {
                ...
                cy.get(TestCasesPage.tcIPHighlightingDetails).should(
                    'contain.text',
                ...
                cy.get(TestCasesPage.tcDENOMHighlightingDetails).should(
                    'contain.text',
...
describe('Highlighting accurately appears for a multiple PC measure', () => {
    ...
    it('Verify highlighting for a measure with multiple PCs', () => {
        ...
        cy.get(MeasureGroupPage.saveMeasureGroupDetails).click()
        cy.wait('@group', { timeout: 60000 }).then((request) => {
            TestData.writeFixture('groupId2', request?.response?.body.id)
        })
        ...
        TestData.readFixture('groupId2').then((secondGroupId) => {
                cy.get('[data-testid="option-' + secondGroupId + '"]')
                    .scrollIntoView()
                    .click({ force: true })

                cy.get('[data-testid="group-coverage-nav-' + secondGroupId + '"]')
                    .contains('IP')
                    .click()
                Utilities.waitForElementVisible('[data-statement-name="Initial Population"]', 35000)
                cy.get('[data-statement-name="Initial Population"]').should(
                    'contain.text',
                    '\ndefine "Initial Population":\nexists "Qualifying Encounters"\n'
                )
                ...
                cy.get('[data-statement-name="Initial PopulationOne"]').should(
                    'contain.text',
                    '\ndefine "Initial PopulationOne":\ntrue\n'
                )
...
describe('Highlighting tab shows Results, Definitions, Functions, and Unused sections', () => {
    ...
    it('Verify tabs in the highlighting sub-menu', () => {
        ...
        cy.get('[data-statement-name="Encounter to Assess Cognition"]').should(
            'contain.text',
        ...
        cy.get(TestCasesPage.tcUNUSEDHightlightingDetails).should(
            'contain.text',
            'define "Patient Reason for Not Performing Assessment of Cognition Using Standardized Tools or Alternate Methods": "unavailable"ResultsNA define "Denominator Exceptions": "unavailable"ResultsNA define "SDE Ethnicity": "unavailable"ResultsNA define "SDE Race": "unavailable"ResultsNA define "SDE Sex": "unavailable"ResultsNA define "SDE Payer": "unavailable"ResultsNA define "track1": "unavailable"ResultsNA define "SDE Ethnicity": "unavailable"ResultsNA define "SDE Payer": "unavailable"ResultsNA define "SDE Race": "unavailable"ResultsNA define "SDE Sex": "unavailable"ResultsNA '
        )
    })
})
...
describe('Highlighting tab sub-sections default to expanded but can be collapsed', () => {
    ...
    it('Result section starts expanded with content and can be collapsed', () => {
        ...
        TestData.readMeasureGroupId().then((fileContents) => {
                ...
                cy.get(TestCasesPage.tcIPHighlightingDetails)
        ...
        cy.get(TestCasesPage.tcFUNCTIONSHighlightingDetails)
            .first()
            .should(
                'contain.text',
...
describe('Highlighting accurately appears for a measure with same Definition in the library', () => {
    beforeEach('Create measure, measure group, test case and login', () => {
        CreateMeasurePage.CreateQICoreMeasureAPI(
            measureName,
            CqlLibraryName,
            measureCQL_withDuplicateLibraryDefinition_wNoUnusedDefine,
            undefined,
            false,
            '2025-01-01',
            '2025-12-31'
    ...
    it('Highlighting is accurate when measure & included library both have definitions with the same name', () => {
        ...
        TestCasesPage.runTestCaseAndOpenHighlighting()

        TestData.readFixture('measureGroupId').then((fileContents) => {
...
describe('Verify highlighting occurs on a newly versioned measure', () => {
    ...
    it('Execute Test Case on a newly versioned measure; verify Measure highlighting happens', () => {
        ...
        TestCasesPage.executeBooleanPassingTestCaseAndOpenHighlighting({
            waitForRunButtonEnabledMs: 30000,
            waitForRunCompletionMs: 90500
        })
...
describe('Verify highlighting occurs on an old versioned measure', () => {
    ...
    */
    ...
    it('Execute Test Case on an old versioned measure; verify Measure highlighting happens', () => {
        ...
        //run test case
        TestCasesPage.runTestCaseAndOpenHighlighting({
            waitForRunButtonEnabledMs: 45500
        })
        ...
        cy.get(TestCasesPage.tcIPHighlightingDetails)
            .find('[data-statement-name="Initial Population"]')
            .should('contain.text', expectedHighlightingText)
        cy.get('[data-ref-id="328"]').should('have.color', '#20744c')
