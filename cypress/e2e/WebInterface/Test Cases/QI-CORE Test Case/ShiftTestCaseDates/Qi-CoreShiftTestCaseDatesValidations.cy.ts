            ...
            cy.readFile('cypress/fixtures/' + currentUser + '/measureId').should('exist').then((id) => {
                cy.readFile('cypress/fixtures/' + currentUser + '/testCaseId').should('exist').then((testCaseId) => {
                    cy.request({
                        ...
                        expect(response.body.json).to.contain('2027-01-01T03:34:10.054Z')
                    })
                })
            })
        })
            ...
            cy.readFile('cypress/fixtures/' + currentUser + '/measureId').should('exist').then((id) => {
                cy.readFile('cypress/fixtures/' + currentUser + '/testCaseId2').should('exist').then((testCaseId2) => {
                    cy.request({
                        ...
                        expect(response.body.json).to.contain('2025-05-13T03:34:10.054+00:00')
                    })
                })
            })
        })
            ...
            cy.readFile('cypress/fixtures/' + currentUser + '/measureId').should('exist').then((id) => {
                cy.readFile('cypress/fixtures/' + currentUser + '/testCaseId').should('exist').then((testCaseId) => {
                    cy.request({
                        ...
                        expect(response.body.id).to.eql(testCaseId)
                        expect(response.body.title).to.eql(testCase1.title)
                        expect(response.body.json).to.contain('2021-01-01T03:34:10.054Z')
                    })
                })
            })
        })
