...
- UI QDM non-owner action-center proof: the reported export scenario in `QDMTestCaseButtons.cy.ts` passes on TEST after translating/saving CQL and creating the Population Criteria through existing APIs before the single alt-user login. The test now waits for execution readiness before clicking, and focused runtime dropped from roughly 45 seconds to 27 seconds.
- UI measure export and population-value proof: the reported Qi-Core and QDM missing-CQL export scenarios pass after synchronizing the measure-list request and keeping their intentional negative export path separate from the success-only action helper. The versioned QDM file export passes locally by saving through `CQLEditorPage.saveCql(...)`, remaining on the newly versioned measure, and waiting for the rerendered edit action center before exporting. All three reported `TestCasePopulationValues.cy.ts` scenarios pass after making the stored test-case row selector the destination-readiness condition in `clickEditforCreatedTestCase(...)`.
- UI smoke/search/version proof: `DeleteMeasure.cy.ts` passes on TEST through the shared delete-confirmation and toast contracts; `QiCoreTestCaseSearch.cy.ts` passes after reopening Test Cases with its search input as destination readiness; and the reported Qi-Core versioned-measure clone/import scenario passes after selecting the newly created case by title instead of assuming Case #1.
- Known product blocker: `RatioPatientTwoIPsWithMOs.cy.ts` now passes CQL-save and Population Criteria tab readiness, resolving the reported missing group-type dropdown. Its downstream Expected/Actual measure-observation failure is a known DEV product bug and should not be masked with test retries or weakened assertions.
...
- reported QDM regression failures are the immediate priority:
  1. `ElementTable.cy.ts`: align CQL-before-Population setup and cross-tab destination readiness with the proven QDM pattern
  2. `QDMCQMExecutionFailureErrorValidations.cy.ts`, `QDMRunExecuteTC.cy.ts`, and `MeasureHighlightingQDM.cy.ts`: handle execution-state, translator, and terminology dependencies as a separate QDM bucket
  3. remaining reported QDM Population Criteria and CQL-editor failures after the proven export/versioning slice
